...
public class TeacherSpecification {
    ...
    private static Specification<Teacher> hasFirstname(String firstname) {
        return ((root, query, cb) -> {
            return (firstname == null ? cb.conjunction() :
                    cb.like(cb.lower(root.get("firstname")), "%" + firstname.toLowerCase() + "%"));
        });
    }

    // // (commented out the below declaration because a more succinct declaration follows it right below)
    // private static Specification<Teacher> hasLastname(String lastname) {
    //     return ((root, query, cb) -> {
    //         return (lastname == null ? cb.conjunction() :
    ...
    // // (commented out the below declaration because a more succinct declaration follows it right below - the check for if region is null is optional since spring boot will automatically call cb.conjunction in that case)
    ...
    private static Specification<Teacher> hasRegion(String region) {
        return (root, query, cb) -> cb.equal(
                cb.lower(root.get("region").get("name")), region.toLowerCase());
    }
